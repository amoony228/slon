...
async def translate_text(input_text:str, to_language:str) -> List[str]: 
    translation_responce = await llm_client.get_ai_answer(f'''translate the following into {to_language}: "{input_text}". 
                                                            The result should be only a line with the translation''')
        
    explanation_response = await llm_client.get_ai_answer(f'''
        Please provide the BRIEFEST POSSIBLE explanation in {to_language} of the puns, jokes, etc. (if any) that were implied in this text: "{input_text}" 
